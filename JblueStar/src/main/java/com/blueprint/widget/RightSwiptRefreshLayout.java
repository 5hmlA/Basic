 ...
 */
public class RightSwiptRefreshLayout extends ViewGroup implements NestedScrollingParent, NestedScrollingChild {
    ...
    protected int mOriginalOffsetRight;
    ...
    private RightSwiptRefreshLayout.OnChildScrollUpCallback mChildScrollUpCallback;
    ...
    private Animation.AnimationListener mRefreshListener = new Animation.AnimationListener() {
        ...
        public void onAnimationEnd(Animation animation){
            if(mRefreshing) {
                ...
                mCurrentTargetOffsetRight = mCircleView.getRight();
            }else {
                reset();
            }
        }
    ...
    void reset(){
        ...
        if(mScale) {
            ...
            setTargetOffsetTopAndBottom(mOriginalOffsetRight-mCurrentTargetOffsetRight);
        }
        mCurrentTargetOffsetRight = mCircleView.getRight();
    }
     ...
     * @param context
     */
    public RightSwiptRefreshLayout(@NonNull Context context){
        this(context, null);
    }
     ...
     * @param attrs
     */
    public RightSwiptRefreshLayout(@NonNull Context context, @Nullable AttributeSet attrs){
        super(context, attrs);
        ...
        mOriginalOffsetRight = mCurrentTargetOffsetRight = -mCircleDiameter;
        moveToStart(1.0f);
    ...
    private void createProgressView(){
        ...
        //        mCircleView = new JDimImageView(getContext());
    ...
    public void setRefreshing(boolean refreshing){
        if(refreshing && mRefreshing != refreshing) {
            ...
            if(!mUsingCustomStart) {
                endTarget = mSpinnerOffsetEnd+mOriginalOffsetRight;
            }else {
                endTarget = mSpinnerOffsetEnd;
            }
            setTargetOffsetTopAndBottom(endTarget-mCurrentTargetOffsetRight);
            mNotify = false;
    ...
    private void setRefreshing(boolean refreshing, final boolean notify){
        if(mRefreshing != refreshing) {
            ...
            mRefreshing = refreshing;
            if(mRefreshing) {
                animateOffsetToCorrectPosition(mCurrentTargetOffsetRight, mRefreshListener);
            }else {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom){
        ...
        mCircleView.layout(( width/2-circleWidth/2 ), mCurrentTargetOffsetRight, ( width/2+circleWidth/2 ),
                mCurrentTargetOffsetRight+circleHeight);
    }
    ...
    /**
     * Set a callback to override {@link RightSwiptRefreshLayout#canChildScrollUp()} method. Non-null
     * callback will return the value provided by the callback and ignore all internal logic.
     *
     * @param callback
     *         Callback that should be called when canChildScrollUp() is called.
     */
    public void setOnChildScrollUpCallback(@Nullable RightSwiptRefreshLayout.OnChildScrollUpCallback callback){
...
//        if(!isEnabled() || mReturningToStart || canChildScrollUp() || mRefreshing || mNestedScrollInProgress) {
        ...
        switch(action) {
            case MotionEvent.ACTION_DOWN:
                setTargetOffsetTopAndBottom(mOriginalOffsetRight-mCircleView.getRight());
    ...
    public void onNestedPreScroll(View target, int dx, int dy, int[] consumed){
        ...
        if(dx>0 && mTotalUnconsumed>0) {
            if(dy>mTotalUnconsumed) {
                consumed[1] = dx-(int)mTotalUnconsumed;
                mTotalUnconsumed = 0;
            }else {
                mTotalUnconsumed -= dx;
                consumed[1] = dx;
    ...
    public void onNestedScroll(final View target, final int dxConsumed, final int dyConsumed, final int dxUnconsumed, final int dyUnconsumed){
        ...
        final int dx = dyUnconsumed+mParentOffsetInWindow[1];
    ...
    private void moveSpinner(float overscrollTop){
        ...
        float slingshotDist = mUsingCustomStart ? mSpinnerOffsetEnd-mOriginalOffsetRight : mSpinnerOffsetEnd;
        float tensionSlingshotPercent = Math.max(0, Math.min(extraOS, slingshotDist*2)/slingshotDist);
        float tensionPercent = (float)( ( tensionSlingshotPercent/4 )-Math.pow(( tensionSlingshotPercent/4 ), 2) )*2f;
        float extraMove = ( slingshotDist )*tensionPercent*2;

        int targetX = mOriginalOffsetRight+(int)( ( slingshotDist*dragPercent )+extraMove );
        ...
        setTargetOffsetTopAndBottom(targetX-mCurrentTargetOffsetRight);
    }
    ...
    private void finishSpinner(float overscrollTop){
        if(overscrollTop>mTotalDragDistance) {
            ...
            animateOffsetToStartPosition(mCurrentTargetOffsetRight, listener);
    ...
    private final Animation mAnimateToCorrectPosition = new Animation() {
        ...
        public void applyTransformation(float interpolatedTime, Transformation t){
            ...
            int endTarget = 0;
            if(!mUsingCustomStart) {
                endTarget = mSpinnerOffsetEnd-Math.abs(mOriginalOffsetRight);
            }else {
                endTarget = mSpinnerOffsetEnd;
            }
            targetTop = ( mFrom+(int)( ( endTarget-mFrom )*interpolatedTime ) );
            int offset = targetTop-mCircleView.getRight();
    ...
    void moveToStart(float interpolatedTime){
        int targetRight = 0;
        targetRight = ( mFrom+(int)( ( mOriginalOffsetRight-mFrom )*interpolatedTime ) );
        int offset = targetRight-mCircleView.getRight();
        setTargetOffsetTopAndBottom(offset);
    }
    ...
    void setTargetOffsetTopAndBottom(int offset){
        ...
        System.out.println(offset+"==============="+getWidth());
//        ViewCompat.offsetLeftAndRight(mCircleView, offset);
    ...
    public interface OnChildScrollUpCallback {
         ...
         */
        boolean canChildScrollUp(@NonNull RightSwiptRefreshLayout parent, @Nullable View child);
    }
}