...
public class BlurHelper {
    ...
    public static Bitmap doBlur(Bitmap sentBitmap, float radius) {
        ...
        if (radius<=6&& Build.VERSION.SDK_INT > 16) {//经测试，radius大于6后，fastBlur效率更高，并且RenderScript在api11以上使用
            Bitmap bitmap = Bitmap
                    .createScaledBitmap(sentBitmap, sentBitmap.getWidth()/SCALE,sentBitmap.getHeight()/SCALE,false);//先缩放图片，增加模糊速度
            final RenderScript rs = RenderScript.create(LibApp.getContext());
    ...
    public static void blur(Bitmap bkg, View view) {
        bkg = BitmapHelper.scaleBitmap(bkg, 1/6f);
        float radius = 20;
        ...
        view.setBackground(new BitmapDrawable(LibApp.getContext().getResources(), doBlur(overlay, radius)));
    }
}
